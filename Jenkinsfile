    ...
    stages {
        ...
        stage('Docker image'){
            ...
            steps {
                // BUILD CONTAINER
                container('dnd'){
                    sh "docker build --tag dawnbreather/feed-quiz:latest ."
                    sh "docker login -u'${IMG_REG_CREDS_USR}' -p'${IMG_REG_CREDS_PSW}'"
                    sh "docker push dawnbreather/feed-quiz:latest"
                }
            }
        }
            ...
            steps {
                container('kubectl') {
                    ...
                    sh '''
                        ...
                        cat /tmp/pods.info
                        grep -E feed-quiz-[a-z0-9]{8,12}-[a-z0-9]{4,6} /tmp/pods.info > /tmp/pods.info.filtered || :
